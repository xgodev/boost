...
description: "GCP Pub/Sub client construction via github.com/xgodev/boost/factory/contrib/cloud.google.com/pubsub/v1 — NewClient with config-driven project ID, lifecycle, defer Close. Apply when wiring Pub/Sub clients in a boost service. Pair with boost-bootstrap-adapter-pubsub (subscriber side) and boost-wrapper-publisher (publish side). STATUS: stub — not yet lapidated via the TDD cycle. Treat as a placeholder routing to boost-core until first concrete usage forces a RED-GREEN-REFACTOR pass."
...
GCP Pub/Sub client construction via github.com/xgodev/boost/factory/contrib/cloud.google.com/pubsub/v1 — NewClient with config-driven project ID, lifecycle, defer Close. Apply when wiring Pub/Sub clients in a boost service. Pair with boost-bootstrap-adapter-pubsub (subscriber side) and boost-wrapper-publisher (publish side).
