...
description: "Driver-agnostic publishing via github.com/xgodev/boost/wrapper/publisher.New(driver) over drivers under wrapper/publisher/driver/contrib/* (Pub/Sub, Kafka, NATS, Goka). Includes the Driver interface contract, ordering keys, attribute mapping, and per-driver config keys. Apply when publishing CloudEvents from a boost service. STATUS: stub — not yet lapidated via the TDD cycle. Treat as a placeholder routing to boost-core until first concrete usage forces a RED-GREEN-REFACTOR pass."
...
Driver-agnostic publishing via github.com/xgodev/boost/wrapper/publisher.New(driver) over drivers under wrapper/publisher/driver/contrib/* (Pub/Sub, Kafka, NATS, Goka). Includes the Driver interface contract, ordering keys, attribute mapping, and per-driver config keys. Apply when publishing CloudEvents from a boost service.
