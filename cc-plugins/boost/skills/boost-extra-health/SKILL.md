...
description: "Health-check primitives at github.com/xgodev/boost/extra/health — checker interface, aggregator, HTTP endpoint. Distinguishes liveness (process up) from readiness (downstream healthy). Apply when adding /health, /readyz, /livez endpoints or registering downstream health checks. STATUS: stub — not yet lapidated via the TDD cycle. Treat as a placeholder routing to boost-core until first concrete usage forces a RED-GREEN-REFACTOR pass."
...
Health-check primitives at github.com/xgodev/boost/extra/health — checker interface, aggregator, HTTP endpoint. Distinguishes liveness (process up) from readiness (downstream healthy). Apply when adding /health, /readyz, /livez endpoints or registering downstream health checks.
