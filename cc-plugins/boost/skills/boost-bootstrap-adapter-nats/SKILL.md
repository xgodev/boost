...
description: "NATS subscriber adapter at github.com/xgodev/boost/bootstrap/function/adapter/contrib/nats-io/nats.go/v1 — subscriber wiring, queue groups, subjects, ack semantics. Note: helper at this layer also hard-codes context.Background() — same workaround pattern as the Pub/Sub adapter applies. Apply when writing NATS function subscribers in a boost service. STATUS: stub — not yet lapidated via the TDD cycle. Treat as a placeholder routing to boost-core until first concrete usage forces a RED-GREEN-REFACTOR pass."
...
NATS subscriber adapter at github.com/xgodev/boost/bootstrap/function/adapter/contrib/nats-io/nats.go/v1 — subscriber wiring, queue groups, subjects, ack semantics. Note: helper at this layer also hard-codes context.Background() — same workaround pattern as the Pub/Sub adapter applies. Apply when writing NATS function subscribers in a boost service.
