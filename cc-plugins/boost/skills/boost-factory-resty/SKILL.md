...
description: "Building outbound HTTP clients via github.com/xgodev/boost/factory/contrib/go-resty/resty/v2 — config-path-based client creation (NewClientWithConfigPath), per-target config namespacing (resty.ConfigAdd), retry policy, timeouts, base URL handling. Apply when writing Go code that calls external HTTP APIs from a boost service. STATUS: stub — not yet lapidated via the TDD cycle. Treat as a placeholder routing to boost-core until first concrete usage forces a RED-GREEN-REFACTOR pass."
...
Building outbound HTTP clients via github.com/xgodev/boost/factory/contrib/go-resty/resty/v2 — config-path-based client creation (NewClientWithConfigPath), per-target config namespacing (resty.ConfigAdd), retry policy, timeouts, base URL handling. Apply when writing Go code that calls external HTTP APIs from a boost service.
