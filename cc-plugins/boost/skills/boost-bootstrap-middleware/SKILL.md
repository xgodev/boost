...
description: "Function middleware composition using github.com/xgodev/boost/bootstrap/function/middleware/{recovery,logger,publisher,ignore_errors}. Canonical order recovery -> logger -> publisher; deadletter routing via the publisher middleware matching on errors.NotValid / errors.Internal type names. Apply when stacking middlewares for a boost function. STATUS: stub — not yet lapidated via the TDD cycle. Treat as a placeholder routing to boost-core until first concrete usage forces a RED-GREEN-REFACTOR pass."
...
Function middleware composition using github.com/xgodev/boost/bootstrap/function/middleware/{recovery,logger,publisher,ignore_errors}. Canonical order recovery -> logger -> publisher; deadletter routing via the publisher middleware matching on errors.NotValid / errors.Internal type names. Apply when stacking middlewares for a boost function.
