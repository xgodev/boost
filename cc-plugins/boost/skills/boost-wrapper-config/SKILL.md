...
description: "Configuration registry via github.com/xgodev/boost/wrapper/config (koanf-based). Covers config.Add (key + default + description), typed accessors (String/Int/Bool/Duration), env override semantics (BOOST_FACTORY_X = boost.factory.x), file providers, koanf transformers. Apply when registering or reading any tunable in a boost service or contrib. STATUS: stub — not yet lapidated via the TDD cycle. Treat as a placeholder routing to boost-core until first concrete usage forces a RED-GREEN-REFACTOR pass."
...
Configuration registry via github.com/xgodev/boost/wrapper/config (koanf-based). Covers config.Add (key + default + description), typed accessors (String/Int/Bool/Duration), env override semantics (BOOST_FACTORY_X = boost.factory.x), file providers, koanf transformers. Apply when registering or reading any tunable in a boost service or contrib.
