...
description: "GCP Pub/Sub subscriber adapter at github.com/xgodev/boost/bootstrap/function/adapter/contrib/cloud.google.com/pubsub/v1 — apubsub.New, NewSubscriber, Subscribe(ctx). Documents the production caveat that helper.go:51 hard-codes context.Background() and the canonical workaround using apubsub.NewSubscriber + signal.NotifyContext + extra/middleware.NewAnyErrorWrapper. Apply when writing Pub/Sub function subscribers that need graceful shutdown. STATUS: stub — not yet lapidated via the TDD cycle. Treat as a placeholder routing to boost-core until first concrete usage forces a RED-GREEN-REFACTOR pass."
...
GCP Pub/Sub subscriber adapter at github.com/xgodev/boost/bootstrap/function/adapter/contrib/cloud.google.com/pubsub/v1 — apubsub.New, NewSubscriber, Subscribe(ctx). Documents the production caveat that helper.go:51 hard-codes context.Background() and the canonical workaround using apubsub.NewSubscriber + signal.NotifyContext + extra/middleware.NewAnyErrorWrapper. Apply when writing Pub/Sub function subscribers that need graceful shutdown.
