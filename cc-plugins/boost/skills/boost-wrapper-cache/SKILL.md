...
description: "Generic Manager[T]-based caching via github.com/xgodev/boost/wrapper/cache — drivers under wrapper/cache/driver/contrib/*, codecs (binary, gob, json, string), plugin chains. Apply when adding a cache layer to a boost service. STATUS: stub — not yet lapidated via the TDD cycle. Treat as a placeholder routing to boost-core until first concrete usage forces a RED-GREEN-REFACTOR pass."
...
Generic Manager[T]-based caching via github.com/xgodev/boost/wrapper/cache — drivers under wrapper/cache/driver/contrib/*, codecs (binary, gob, json, string), plugin chains. Apply when adding a cache layer to a boost service.
