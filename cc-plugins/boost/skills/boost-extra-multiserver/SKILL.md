...
description: "Multi-server orchestration at github.com/xgodev/boost/extra/multiserver — coordinated start/stop for HTTP + gRPC + metrics + health side-cars. Apply when a single binary needs to expose more than one network listener. STATUS: stub — not yet lapidated via the TDD cycle. Treat as a placeholder routing to boost-core until first concrete usage forces a RED-GREEN-REFACTOR pass."
...
Multi-server orchestration at github.com/xgodev/boost/extra/multiserver — coordinated start/stop for HTTP + gRPC + metrics + health side-cars. Apply when a single binary needs to expose more than one network listener.
