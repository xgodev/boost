package main

import (
	"context"
	"os"

	"github.com/xgodev/boost/config"
	"github.com/xgodev/boost/factory/google.golang.org/grpc.v1/server"
	"github.com/xgodev/boost/factory/google.golang.org/grpc.v1/server/examples/examplesvc/pb"
	"github.com/xgodev/boost/factory/google.golang.org/grpc.v1/server/plugins/contrib/americanas-go/log.v1"
	ilog "github.com/xgodev/boost/factory/xgodev/boost.v1/log"
	alog "github.com/xgodev/boost/log"
)

func init() {
	os.Setenv("IGNITE_LOGRUS_CONSOLE_LEVEL", "TRACE")
}

func main() {

	ctx := context.Background()

	config.Load()

	ilog.New()

	options, _ := server.NewOptions()
	options.Port = 8080

	srv := server.NewServerWithOptions(ctx, options, log.Register)

	pb.RegisterExampleServer(srv.ServiceRegistrar(), NewService())

	srv.Serve(ctx)
}

type Service struct {
	pb.UnimplementedExampleServer
}

func (h *Service) Test(ctx context.Context, request *pb.TestRequest) (*pb.TestResponse, error) {

	logger := alog.FromContext(ctx)

	logger.Infof(request.Message)

	return &pb.TestResponse{Message: "hello world"}, nil
}

func NewService() pb.ExampleServer {
	return &Service{}
}
